...
int Tcp::setConnection(int numberOfClients, void *(*f)(void *)){
	threads = new pthread_t[numberOfClients];
...
***********************************************************************/
int Tcp::sendData(std::string data) {
	int data_len = (int) data.length();
	const char * datas = data.c_str();
	int sent_bytes = (int) send(this->isServer ? this->descriptorCommunicateClient
                                               : this->socketDescriptor, datas, (size_t) data_len, 0);
	if (sent_bytes < 0) {
		//return an error represent error at this method
		return ERROR_SEND;
	}
...
int Tcp::reciveData(char* buffer, int size) {
	int read_bytes = (int) recv(this->isServer ? this->descriptorCommunicateClient
                                               : this->socketDescriptor, buffer, (size_t) size, 0);
	//checking the errors
	if (read_bytes == 0) {
